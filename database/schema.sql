...
-- VERSÃO CORRIGIDA — alinhada com o código do backend
-- =============================================
--
-- Alterações em relação ao script original (bd.md):
--   1. estado_conta / estado_permanente / estado_sala / estado_exibicao:
--      DEFAULTs corrigidos para os valores que o código realmente usa,
--      e CHECK constraints adicionadas (antes eram TEXT livre).
--   2. compras: adicionadas colunas data_cancelamento e valor_reembolsado
--      (usadas em várias rotas, mas ausentes do script original — causavam
--      erro de "column does not exist" em runtime).
--   3. sessoes: adicionadas colunas data_atualizacao e data_cancelamento
--      (mesmo motivo).
--   4. lugares_ocupados.status: CHECK constraint adicionada.
-- =============================================
...
telefone VARCHAR,
tipo_utilizador TEXT NOT NULL CHECK (tipo_utilizador IN ('cliente', 'funcionario', 'administrador')),
data_cadastro TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
ultimo_acesso TIMESTAMP,
estado_conta TEXT DEFAULT 'ativo' CHECK (estado_conta IN ('ativo', 'inativo', 'bloqueado')),
foto_url VARCHAR
);
...
estado_exibicao TEXT DEFAULT 'disponivel' CHECK (estado_exibicao IN ('disponivel', 'indisponivel', 'brevemente')),
...
estado_sala TEXT DEFAULT 'ativa' CHECK (estado_sala IN ('ativa', 'inativa', 'manutencao')),
coluna INT,
fila INT
);
...
fileira VARCHAR,
numero INT,
estado_permanente TEXT DEFAULT 'ativo' CHECK (estado_permanente IN ('ativo', 'inativo', 'manutencao')),
codigo INT,
...
preco NUMERIC(10,2),
estado_sessao TEXT DEFAULT 'agendada' CHECK (estado_sessao IN ('agendada', 'em_andamento', 'concluida', 'cancelada')),
criado_por VARCHAR,
observacoes TEXT,
...
estado_pagamento TEXT DEFAULT 'pendente' CHECK (estado_pagamento IN ('pendente', 'aprovado', 'cancelado')),
...
estado_uso TEXT DEFAULT 'nao_usado' CHECK (estado_uso IN ('nao_usado', 'usado', 'cancelado')),
data_uso TIMESTAMP,
...
id_sessao VARCHAR NOT NULL,
status VARCHAR NOT NULL CHECK (status IN ('pendente', 'ocupado', 'reservado', 'cancelado')),
id_sala VARCHAR,
